...
import chainflow_pycamunda.casedef
import chainflow_pycamunda.decisiondef
import chainflow_pycamunda.decisionreqdef
...
class Deployment:
    ...
    def load(cls, data: typing.Mapping[str, typing.Any]) -> Deployment:
        ...
        if data['deploymentTime'] is not None:
            deployment.deployment_time = chainflow_pycamunda.base.from_isoformat(data['deploymentTime'])
...
class DeploymentWithDefinitions:
    ...
    links: typing.List[chainflow_pycamunda.resource.Link]
    id_: str
    name: str
    source: str
    deployed_process_definitions: typing.Dict[str, chainflow_pycamunda.processdef.ProcessDefinition]
    deployed_case_definitions: typing.Dict[str, chainflow_pycamunda.casedef.CaseDefinition]
    deployed_decision_definitions: typing.Dict[str, chainflow_pycamunda.decisiondef.DecisionDefinition]
    deployed_decision_requirements_definitions: typing.Dict[
        str, chainflow_pycamunda.decisionreqdef.DecisionRequirementsDefinition
    ]
    tenant_id: str = None
    deployment_time: dt.datetime = None
    ...
    def load(cls, data: typing.Mapping[str, typing.Any]) -> DeploymentWithDefinitions:
        if data['deployedProcessDefinitions'] is not None:
            deployed_process_definitions = {
                name: chainflow_pycamunda.processdef.ProcessDefinition.load(process_definition)
        ...
        if data['deployedCaseDefinitions'] is not None:
            deployed_case_definitions = {
                name: chainflow_pycamunda.casedef.CaseDefinition.load(case_definition)
        ...
        if data['deployedDecisionDefinitions'] is not None:
            deployed_decision_definitions = {
                name: chainflow_pycamunda.decisiondef.DecisionDefinition.load(decision_definition)
        ...
        if data['deployedDecisionRequirementsDefinitions'] is not None:
            deployed_decision_requirements_definitions = {
                name: chainflow_pycamunda.decisionreqdef.DecisionRequirementsDefinition.load(dec_req_def)
        ...
        return cls(
            links=[chainflow_pycamunda.resource.Link.load(link) for link in data['links']],
            id_=data['id'],
            name=data['name'],
            source=data['source'],
            ...
            tenant_id=data['tenantId'],
            deployment_time=chainflow_pycamunda.base.from_isoformat(data['deploymentTime'])
        )
...
class GetList(chainflow_pycamunda.base.CamundaRequest):
    ...
    include_deployments_without_tenant_id = QueryParameter(
        'includeDeploymentsWithoutTenantId', provide=chainflow_pycamunda.base.value_is_true
    )
    ...
    def __call__(self, *args, **kwargs) -> DeploymentWithDefinitions:
        ...
        try:
            response = requests.request(
                method=chainflow_pycamunda.base.RequestMethod.POST.value,
                url=self.url,
            ...
            raise chainflow_pycamunda.PyCamundaException(exc)
        if not response:
...
class GetResources(chainflow_pycamunda.base.CamundaRequest):
...
class GetResource(chainflow_pycamunda.base.CamundaRequest):
