...
class _Criteria(chainflow_pycamunda.base.CamundaRequest):
    ...
    def add_user_criteria(
        ...
        involved_user: str = ...,
        unassigned: bool = ...,
        delegation_state: typing.Union[str, chainflow_pycamunda.task.DelegationState] = ...
    ) -> None:
        ...
        if delegation_state is not Ellipsis:
            ...
            if delegation_state is not None:
                self.delegation_state = chainflow_pycamunda.task.DelegationState(delegation_state)
...
class Execute(_Criteria):
    ...
    def __call__(
        self, *args, **kwargs
    ) -> typing.Union[chainflow_pycamunda.task.Task, typing.Tuple[chainflow_pycamunda.task.Task]]:
        """Send the request."""
        ...
        if self.single_result:
            return chainflow_pycamunda.task.Task.load(response.json())
        return tuple(chainflow_pycamunda.task.Task.load(task_json) for task_json in response.json())


class ExecuteCount(chainflow_pycamunda.base.CamundaRequest):
...
class Options(chainflow_pycamunda.base.CamundaRequest):
    ...
    def __call__(self, *args, **kwargs):
        ...
        return chainflow_pycamunda.resource.ResourceOptions.load(response.json())
