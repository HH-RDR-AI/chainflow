...
import chainflow_pycamunda.request
...
class CamundaRequest(chainflow_pycamunda.request.Request):
...
def _raise_for_status(response: requests.Response) -> None:
    ...
    if response.status_code == 400:
        raise chainflow_pycamunda.BadRequest(message)
    if response.status_code == 401:
        raise chainflow_pycamunda.Unauthorized(message)
    elif response.status_code == 403:
        raise chainflow_pycamunda.Forbidden(message)
    elif response.status_code == 404:
        raise chainflow_pycamunda.NotFound(message)

    raise chainflow_pycamunda.NoSuccess(response.text)
