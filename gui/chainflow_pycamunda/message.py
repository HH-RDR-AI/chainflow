...
import chainflow_pycamunda.execution
...
class MessageCorrelationResult:
    ...
    process_instance: chainflow_pycamunda.processinst.ProcessInstance = None
    execution: chainflow_pycamunda.execution.Execution = None
    variables: typing.Tuple[chainflow_pycamunda.variable.Variable] = None
    ...
    def load(cls, data: typing.Mapping[str, typing.Any]) -> MessageCorrelationResult:
        ...
        if message_result.result_type == ResultType.process_definition:
            message_result.process_instance = chainflow_pycamunda.processinst.ProcessInstance.load(
                data['processInstance']
            )
        elif message_result.result_type == ResultType.execution:
            message_result.execution = chainflow_pycamunda.execution.Execution.load(data['execution'])
        ...
        try:
            ...
            message_result.variables = tuple(
                chainflow_pycamunda.variable.Variable.load(variable_json) for variable_json in variables
            )

        return message_result


class _Correlate(chainflow_pycamunda.base.CamundaRequest):

    message_name = BodyParameter('messageName')
    business_key = BodyParameter('businessKey')
    tenant_id = BodyParameter('tenantId')
    without_tenant_id = BodyParameter('withoutTenantId', provide=chainflow_pycamunda.base.value_is_true)
